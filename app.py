...
def search_page():
    ...
    with col1:
        st.markdown("#### 🔍 **パターン1: 落とし物探し**")
        st.markdown("""
        1. **ギャラリー**から画像を選ぶ（これが落とし物です）
        2. その画像を見た情報で**検索**してください  
           （元の説明文と同じである必要はありません）
        3. 上位10件が表示されるので、最初に決めた画像があれば**「✅ 正解」**ボタンを、なければ**「❌ 正解なし」**ボタンを押してください
        """)
    
    with col2:
        st.markdown("#### 🆓 **パターン2: フリー検索**")
        st.markdown("""
        1. **自由なテキスト**で検索してみてください
        2. 検索結果を確認した後、最後に一番下の**「フリー検索完了」**ボタンを押して送信します
