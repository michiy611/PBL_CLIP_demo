...
def search_page():
    ...
    with col1:
        ...
        **手順：**
        1. 左側の「🖼️ ギャラリー」タブを開く
        2. 画像を1つ選んで覚える（これが落とし物です）
        3. この「🔍 画像検索」タブに戻る
        4. 覚えた画像の特徴を自分の言葉（元の説明と同じである必要なし）で検索してみる
        5. 検索結果の中に選んだ画像があったら「✅ 正解」ボタン
        6. なかったら最下段の「❌ 正解なし」ボタンを押す
        
        💡 **お願い：** データ収集の為、カテゴリ毎に2回以上使っていただきたいです。 
        """)
    ...
    with col2:
        ...
        st.markdown("""
        **手順：**
        1. 思いついたキーワードで自由に検索してみる
        2. 検索結果を見る
        3. 最後に最下段の「🆓 フリー検索」ボタンを押して完了
        
        💡 **目的：** どんな言葉で何が見つかるかを調べます
        """)
